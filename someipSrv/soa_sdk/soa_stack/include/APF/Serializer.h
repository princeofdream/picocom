#ifndef _APF_SERIALIZER_H_
#define _APF_SERIALIZER_H_
...
public:
    Deserializer(uint8_t const* data, uint32_t size) {
    ...
    Deserializer(ara::core::Span<uint8_t const> data) {
...
public:
    ...
    template <typename T>
    Deserializer& seekg(T size, Deserializer& inputBuffer, SEEK_BUFFER seek_flag) {
        ...
        case SEEK_BUFFER::BUFFER_CUR:
            if (((size > 0) && ((uint32_t)(inputBuffer.m_buff.end() - inputBuffer.m_current_pos) < (uint32_t)size)) 
                || ((size < 0) && ((uint32_t)(inputBuffer.m_current_pos - inputBuffer.m_buff.begin()) < (uint32_t)abs(size)))){
    ...
    friend Deserializer& operator >> (Deserializer& inputBuffer, T& _data) {
        if (!inputBuffer.IsStatusOK()) {
            return inputBuffer;
        }
    ...
    friend Deserializer& operator >> (Deserializer& inputBuffer, ara::core::String& str) {
        if (!inputBuffer.IsStatusOK()) {
            return inputBuffer;
        }
     ...
     */
    friend Deserializer& operator >> (Deserializer& inputBuffer, ara::core::u16String& str) {
        ...
        for (size_t i = 0; i < distance_val / 2; i++) {
            ...
            inputBuffer >> value;
            (bom == 0xFFFE) ?
                value = EndianTrait<EndianType::kLittleEndian>::convert(value) :
                value = EndianTrait<EndianType::kBigEndian>::convert(value);
            str.push_back(value);
        }
    ...
    friend Deserializer& operator >> (Deserializer& inputBuffer, std::array<T, N>& ary) {
        if (!inputBuffer.IsStatusOK()) {
            return inputBuffer;
        }
    ...
    template <typename T>
    friend Deserializer& operator >> (Deserializer& _deserilizer, std::vector<T>& _data) {
        _data.insert(_data.end(), _deserilizer.m_current_pos, _deserilizer.m_buff.end());
...
#if 0
    friend Deserializer& operator >> (Deserializer& inputBuffer, ara::core::Vector<uint8_t>& vec) {
...
class DeserializerBin :public Deserializer {
    union UINT16TYPE
    {
        uint8_t ch[2];
        uint16_t idata;
...
public:
    ...
    DeserializerBin(uint8_t* data, uint32_t size) :Deserializer(data, size) {

    }

    DeserializerBin(ara::core::Span<uint8_t const> data) :Deserializer(data) {
    }

    friend Deserializer& operator >> (DeserializerBin& inputBuffer, uint16_t& idata) {
        if (!inputBuffer.IsStatusOK()) {
            return inputBuffer;
        }
        ...
        idata = u16typ.idata;
        inputBuffer.m_current_pos += 2;
        //printf("-----DeserializerBin uint16_t-----\r\n");
        return inputBuffer;
    }
...
public:
    ...
    template <typename T, typename = typename std::enable_if<std::is_arithmetic<T>::value | std::is_enum<T>::value>::type>
    //template <typename T, typename = typename std::enable_if_t<std::is_integral<T>::value>>
    friend Serializer& operator << (Serializer& outputBuffer, T _data) {
    ...
    friend Serializer& operator << (Serializer& outputBuffer, ara::core::String str) {
        if (!outputBuffer.isStatusOK()) {
     ...
     */
    friend Serializer& operator << (Serializer& outputBuffer, ara::core::u16String str) {
        uint16_t bom = 0xFEFF;
        ...
        for (size_t i = 0; i < str.length(); i++) {
            (bom == 0xFFFE)?
                outputBuffer << EndianTrait<EndianType::kLittleEndian>::convert(str[i]):
                outputBuffer << EndianTrait<EndianType::kBigEndian>::convert(str[i]);
        }
        //String ends with "\0"
    ...
    friend Serializer& operator << (Serializer& outputBuffer, ara::core::Span<const uint8_t> ary) {
        if (!outputBuffer.isStatusOK()) {
            //printf("encode data err(array). REMOTECTRLSRV_DECODE_STATUS_BUFFER_OVER_FLOW!!!\n");
            return outputBuffer;
        }
        std::copy(ary.begin(), ary.end(), std::back_inserter(outputBuffer.m_buffer));
    ...
    template<typename T,size_t N>
    friend Serializer& operator << (Serializer& outputBuffer, std::array<T,N> ary) {
        if (!outputBuffer.isStatusOK()) {
            //printf("encode data err(array). REMOTECTRLSRV_DECODE_STATUS_BUFFER_OVER_FLOW!!!\n");
            return outputBuffer;
        }
    ...
    friend Serializer& operator << (Serializer& outputBuffer, Serializer& out2) {
        if (!outputBuffer.isStatusOK()) {
            //printf("encode data err(string). REMOTECTRLSRV_DECODE_STATUS_BUFFER_OVER_FLOW!!!\n");
            return outputBuffer;
        }
    ...
    /**add end**/
#if 0
    friend Serializer& operator << (Serializer& outputBuffer, ara::core::Vector<uint8_t>& vec) {
...
public:
    ...
    friend Serializer& operator << (Serializer& output_buffer, BinInt<T, E> const& _data) {
        output_buffer << EndianTrait<E>::convert(_data.m_value);
        return output_buffer;
    }

    friend Deserializer& operator >> (Deserializer& input_buffer, BinInt<T, E>& _data) {
...
public:
    ...
    friend Deserializer& operator >> (Deserializer& _input_buffer, Map& _data) {
    ...
    friend Serializer& operator << (Serializer& output_buffer, Map& _data) {
