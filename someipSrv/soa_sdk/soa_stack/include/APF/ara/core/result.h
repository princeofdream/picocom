 ...
 * Author : Even Lee
 * 
 * Implemented by std::variant of C++17
 * 
 * 2021-5-25
 * 
 */
...
public:
	...
	Result(T const & t) : m_has_error(false), m_contents(t) {
		// m_value = std::make_unique(t);
	}

    //template <typename U>
    //Result(U const& t) : m_has_error(false) {
...
public:
	...
	Result const & operator = (Result const & other) {
    ...
    Result const & operator = (Result && other) {
		m_has_error = other.m_has_error;
        m_contents = std::move(other.m_contents);
	...
	void EmplaceValue(Args && ... args) {
		m_has_error = false;
        m_contents = ara::core::Variant<T, E>{nonstd::in_place_index<T>, std::forward<Args>(args)... };
	}
	...
	void EmplaceError(Args && ... args) {
		m_has_error = true;
        m_contents = ara::core::Variant<T, E>{ nonstd::in_place_index<E>, std::forward<Args>(args)... };
	}
    ...
    //void Swap(Result& other) noexcept(std::is_nothrow_constructible<T>::value && std::is_nothrow_assignable<T>::value);
...
public:
    ...
    // \note : TODO: -issue doc contains  T const & Value() const &; & is unnecessary.
	T && Value() {
		return std::move(ara::core::get<T>(m_contents));
	}
	...
	E const & Error() const & {
		return ara::core::get<E>(m_contents);
	}
    ...
    // \note : TODO: -issue doc contains  T const & Error() const &; & is unnecessary.
	E && Error() && {
		return std::move(ara::core::get<E>(m_contents));
	}
    ...
    template <typename U>
    typename std::enable_if<std::is_convertible<U, T>::value, T>::type const& ValueOr(U && defaultValue) const & {
		if (m_has_error) {
			return defaultValue;
		}
		return ara::core::get<T>(m_contents);
    ...
    template <typename F>
    T const & Resolve(F && f) const {
        if (HasValue()) {
            return std::get<T>(m_contents);
        }
        
        return f(std::get<E>(m_contents));
    }
...
private:
    bool               m_has_error;
	ara::core::Variant<T, E> m_contents;
};
