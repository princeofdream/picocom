...
#define byte_lite_VERSION  byte_STRINGIFY(byte_lite_MAJOR) "." byte_STRINGIFY(byte_lite_MINOR) "." \
            byte_STRINGIFY(byte_lite_PATCH)
...
#define byte_USES_STD_BYTE  ( (byte_CONFIG_SELECT_BYTE == byte_BYTE_STD) || \
                                ((byte_CONFIG_SELECT_BYTE == byte_BYTE_DEFAULT) && byte_HAVE_STD_BYTE) )
...
{
    return to_byte( static_cast<unsigned char>(~to_uchar( b )) );
}
