#ifndef _APF_SYNCHRONIZED_H
#define _APF_SYNCHRONIZED_H
...
#define APF_DECLVAL(...) std::declval<__VA_ARGS__>()

    template <typename F>
    struct traits {
        template <typename... A>
        using result = decltype( //
            APF_DECLVAL(F&&)(APF_DECLVAL(A&&)...));
        template <typename... A>
...
APF_INLINE_VARIABLE constexpr bool kSynchronizedMutexIsUnique = false;
template <typename Mutex>
APF_INLINE_VARIABLE constexpr bool kSynchronizedMutexIsUnique<
    decltype(void(std::declval<Mutex&>().lock())),
    Mutex> = true;

template <typename, typename Mutex>
APF_INLINE_VARIABLE constexpr bool kSynchronizedMutexIsShared = false;
template <typename Mutex>
APF_INLINE_VARIABLE constexpr bool kSynchronizedMutexIsShared<
    decltype(void(std::declval<Mutex&>().lock_shared())),
    Mutex> = true;

template <typename, typename Mutex>
APF_INLINE_VARIABLE constexpr bool kSynchronizedMutexIsUpgrade = false;
template <typename Mutex>
APF_INLINE_VARIABLE constexpr bool kSynchronizedMutexIsUpgrade<
...
template <typename Mutex>
APF_INLINE_VARIABLE constexpr SynchronizedMutexLevel kSynchronizedMutexLevel =
...
//do not open it, need C++17 compier
//template <>
...
class SynchronizedBase<Subclass, detail::SynchronizedMutexLevel::Shared> {
 private:
  ...
  using LockedPtr_ = ::apf::sync::LockedPtr<T, P>;
 ...
 */
template <class T, class Mutex = std::mutex/*SharedMutex*/>
struct Synchronized : public SynchronizedBase<
 ...
 public:
  ...
  explicit Synchronized(ara::core::in_place_t, Args&&... args)
 ...
 private:
  ...
  friend class apf::sync::LockedPtrBase;
  template <class LockedType, class LockPolicy>
  friend class apf::sync::LockedPtr;
...
class LockedPtr {
 ...
 private:
  ...
  APF_ERASE static LT doLock(MT& mutex) {
 ...
 * using one of the four strategies - apf::sync::lock (exclusive acquire for
 * exclusive only mutexes), apf::sync::rlock (shared acquire for shareable
 * mutexes), apf::sync::wlock (exclusive acquire for shareable mutexes) or
 * apf::sync::ulock (upgrade acquire for upgrade mutexes) (see above)
 *
 ...
 *
 *    apf::sync::lock(one, two, three);
 *
 * To make it work with apf::sync::Synchronized you have to specify how you want
 * the locks to be acquired, use the apf::sync::wlock(), apf::sync::rlock(),
 * apf::sync::ulock() and apf::sync::lock() helpers defined below
 *
 *    auto [one, two] = lock(apf::sync::wlock(a), apf::sync::rlock(b));
 *
 * Note that you can/must avoid the apf::sync:: namespace prefix on the lock()
 ...
 *
 * use lock() with apf::sync::wlock(), apf::sync::rlock() and apf::sync::ulock() for
...
#define SYNCHRONIZED(...)                                                 \
  ...
  if (bool SYNCHRONIZED_VAR(state) = false) {                             \
    (void)::apf::sync::detail::SYNCHRONIZED_macro_is_deprecated{};            \
  } else                                                                  \
