...
{
        char buf[CONFIG_PROCMGR_MAXLEN];
		u_int8_t get_proto;
		...
		plogd("src: %s, \t", inet_ntoa(src_addr));
		plogd("dst: %s. \t ", inet_ntoa(dst_addr));
		plogd("%s\n", buf);
