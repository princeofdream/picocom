...
As its name suggests, *ppcom* is a minimal dumb-terminal emulation
...
For a description of ppcom's operation, its command line options,
and usage examples, see the manual page included in the source
distribution as "ppcom.1", and also html-ized as "ppcom.1.html".

People who have contributed to ppcom, by offering feature
...
Change into ppcom's source directory and say:

    make

This will be enough to compile ppcom for most modern Unix-like
...
kilobytes. Then you can copy the ppcom binary, as well as the
...
Again, this is not strictly necessary. You can run ppcom and read
its man-page directly from the source directory.

If something goes wrong and ppcom can't compile cleanly, or if it's
...
You can start ppcom with its default option values (default serial
port settings) like this:

    ppcom /dev/ttyS0

If you have not installed the ppcom binary to a suitable place, then
...
port's device node. To overcome this you can run ppcom as root:

    sudo ppcom /dev/ttyS0
...
desired values using ppcom's command line options. For example, to
set the baud-rate to 115200bps (the default is 9600bps), and enable
hardware flow-control (RTS/CTS handshake) you can say:

    ppcom -b 115200 -f h /dev/ttyS0

or:

    ppcom --baud 115200 --flow h /dev/ttyS0

To see all available options run ppcom like this:

    ppcom --help

Once ppcom starts, it initializes the serial port and prints the
message:
...
then nothing that you type in ppcom will appear on your
terminal. This is normal.

To exit ppcom you have to type:
...
ppcom that the next character typed is to be interpreted as a
command to ppcom itself (in this case the exit command) and not to
be sent-down to the serial port. There are several other commands
(other than `C-q`, `x`), all prefixed by `C-q`.

Next you should take a look at the very detailed ppcom manual
page. It can be accessed like this (assuming you are inside the
ppcom distribution source directory):
...
Thanks for using ppcom

## Custom Bash completion

Starting with release 3.2, ppcom includes support for custom
...
ppcom*. Here's how you can enable it, if you wish.

To manually enable custom completion support you need to source the
file (custom completion script):

    <ppcom source dir>/bash_completion/ppcom

Assuming you are inside the ppcom source directory, you can do it
like this:

    . ./bash_completion/ppcom
...
1. If you are running a relatively modern Debian or Ubuntu or other
   ...
   the ppcom source directory, something like this will do it:

       sudo cp ./bash_completion/ppcom /etc/bash_completion.d/

   This will enable custom completion support for ppcom, globaly
   (for all Bash-shell users).

   *NOTICE:* If you have another version of ppcom already installed,
   there may already be a `ppcom` completion script in
...
2. If you want to automatically enable support *only for the current
   ...
   Copy the ppcom completion script to the directory you
   created. Assuming you are inside the ppcom source directory:

       cp ./bash_completion/ppcom ~/.bash_completion.d
...
You can use *ppcom* to patch-together a very simple, *very
...
simple as having the users say (from their remote workstations):
    ...
    $ ssh -t user@termbox ppcom -b 115200 /dev/ttyS0

Or make a convenient script/alias for this. Remember the `-t` switch
which instructs ssh to create a pseudo-tty, otherwise ppcom won't
work.

What if you *don't* want to give users shell-access to termbox? Then
you can use ppcom in a setup like the one described below. Just
...
Copy the ppcom binary in `~termbox/bin` (if you don't have it
globally installed):

    $ cp /path/to/ppcom ./bin
...
`~termbox/bin` should look like this:
    ...
    $ ls -l ./bin
    -rwxrwxr-x 1 termbox termbox 102128 Aug 29 13:56 ppcom*
...
for the _ttyS0_ account, since ppcom will not read or write any
...
- If the default port settings you specified as command-line arguments
  to ppcom in `~termbox/bin/ttySx` do not match the settings of the
  device connected to the port, the user can easily change them from
  within ppcom, using ppcom commands.

- If a second user tries to remotely access the same port, at the same
  time, ppcom won't let him (ppcom will find the port locked and
  exit).

- In the example `~termbox/bin/ttySx` scripts we have completely
  disabled the send- and receive-file ppcom commands. This
  guarantees that ppcom won't execute any external commands. If you
  ...
  `--receive-cmd` ppcom command-line options (provided, of-course,
...
that the underlying driver can handle this. Since release 2.0, ppcom
can be compiled with custom baudrate support for some systems. Since
release 3.1 ppcom is compiled with support enabled *by default* on
...
support to be *enabled* by compiling ppcom like this:
...
When ppcom is compiled with custom baudrate support *on Linux*, it
...
(though they should). In order to be able to use ppcom even in this
case, and without recompiling it, you can disable the custom baudrate
support at runtime, and force ppcom to use the "old" ioctls. To do
this (starting with release 3.2) just define the environment variable
`NO_CUSTOM_BAUD` before running ppcom. Something like this:

    NO_CUSTOM_BAUD=1 ppcom ...

This only applies to Linux, and to ppcom binaries that have been
